...
pub const Manager = struct {
    ...
    inline fn run_next_or_exit(self: *Manager, sp: *TrapContext) noreturn {
        ...
        context.* = TrapContext{ .x = blk: {
            var x: @TypeOf(context.x) = undefined;
            x[2] = @ptrToInt(&user_stack) + @sizeOf(@TypeOf(user_stack));
            break :blk x;
        }, .sepc = 0x80400000, .sstatus = blk: {
            var x: usize = asm ("csrr %[x], sstatus"
                : [x] "=r" (-> usize),
            );
            x &= ~@as(usize, 0x100);
            break :blk x;
        } };
        {
            ...
            const dst_p = @intToPtr([*]u8, 0x80400000);
            const dst = dst_p[0..len];
            const src_p = @intToPtr([*]const u8, self.app[self.current][0]);
            const src = src_p[0..len];
            @memcpy(dst, src);
...
pub fn trap_handle(trap_context: *TrapContext) callconv(.C) *TrapContext {
    ...
    log.debug("trap handle {{ cause: {}, value: 0x{x}, sepc: 0x{x} }}", .{
        scause,
        stval,
        sepc,
    });
